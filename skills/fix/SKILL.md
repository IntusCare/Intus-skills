...
| `/code-review` findings comment — a checklist, per `/pull-request` | The line is `- [ ]` | The line is `- [x]`, or the whole comment `isMinimized` (superseded — `/pull-request` hides the previous one each run) |
...
`/pull-request` is what refreshes the review: it re-runs `/code-review` against the updated diff, hides the previous findings comment as outdated, and posts a new checklist. That is what makes iteration N+1 see *new* findings rather than the old list.
