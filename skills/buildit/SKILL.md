...
- **Stage 2 gets `--unattended`,** which changes what a UI ticket produces. `/i:test-first` hands E2E to the repo's E2E workflow (`docs/intus-skills/test-layers.md`). That workflow may stop at a human gate and then run its tests to green: a prompt this chain can't answer, then a suite that isn't red. Unattended, it produces the E2E **plan** only. The chain still delivers the other layers red; the E2E build is a follow-up a human runs. Don't "fix" this by passing a skip-the-gate flag (such as `--yes`) through from here: green E2E tests entering stage 3 would be committed as though the implementation had earned them.
